...
int main()
{
    ...
    if(file.is_open())
    {
        ...
        while(file.read(reinterpret_cast<char*>(&tempOwner), sizeof(OwnerOfCar)))
        {
    ...
    ofstream file_output("OWNERS.dat", ios::binary); 
    if(file_output.is_open())
    {
        for(auto& Owners : carOwners)
        {
            file_output.write(reinterpret_cast<const char*>(&Owners), sizeof(OwnerOfCar));
        }
